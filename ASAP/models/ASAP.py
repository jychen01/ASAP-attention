...
from layers.ASAP_backbone import ASAP_backbone
from layers.ASAP_layers import series_decomp
...
class Model(nn.Module):
    def __init__(self, configs, max_seq_len: Optional[int] = 1024, d_k: Optional[int] = None, d_v: Optional[int] = None,
        ...
        rwr_prob = configs.rwr_prob
        iteration = configs.iteration
        _attn_dropout = configs._attn_dropout
        sigma = configs.sigma
        ...
        if self.decomposition:
            ...
            self.model_trend = ASAP_backbone(c_in=c_in, sigma=sigma, rwr_prob=rwr_prob, season=None, iteration=iteration, _attn_dropout=_attn_dropout,
                                                 time_window=time_window, context_window=context_window, target_window=target_window,
                                                 patch_len=patch_len, stride=stride, max_seq_len=max_seq_len, n_layers=n_layers, d_model=d_model,
                                                 ...
                                                 subtract_last=subtract_last, verbose=verbose, Trend=True, **kwargs)
            self.model_res = ASAP_backbone(c_in=c_in, sigma=sigma, rwr_prob=rwr_prob, season=season, iteration=iteration, _attn_dropout=_attn_dropout,
                                               time_window=time_window, context_window=context_window, target_window=target_window,
                                               ...
                                               subtract_last=subtract_last, verbose=verbose, Trend=False, **kwargs)
        else:
            self.model = ASAP_backbone(c_in=c_in, sigma=sigma, rwr_prob=rwr_prob, season=season, iteration=iteration, _attn_dropout=_attn_dropout,
                                           time_window=time_window, context_window=context_window, target_window=target_window,
                                           patch_len=patch_len, stride=stride,
                                           max_seq_len=max_seq_len, n_layers=n_layers, d_model=d_model,
                                           n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
